...
AIMG — B2B2C REST API для обработки изображений с помощью ИИ. Партнёры интегрируют API в свои приложения, отправляют задачи на обработку (удаление фона, генерация изображений и т.д.) и получают результаты.

**Основные возможности:**

- Генерация изображений по текстовому описанию (text-to-image)
- Удаление фона с изображений (remove background)
- Кредитная система оплаты с двухфазным списанием
- Уведомления о результатах через вебхуки
...
Все эндпоинты `/v1/*` возвращают ответы в унифицированном конверте:
...
> **Исключение:** `GET /health` возвращает ответ напрямую, без конверта.
...
1. **Проверяйте `success`** в конверте ответа перед использованием `data`
...
3. **Поллите с задержкой** — начинайте с 1–2с, увеличивайте до 5с; не опрашивайте чаще 1 раза в секунду
